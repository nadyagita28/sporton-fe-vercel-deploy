...
const CategoryModal = ({isOpen, onClose, onSuccess, category}: TCategoryModalProps) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.error(isEditMode ? "Failed to update category" : "Failed to created category");
            toast.error(isEditMode ? "Failed to update category" : "Failed to created category");
        } finally {
                ...
                <div className="flex gap-7">
                    ...
                    <div className="flex flex-col gap-4 w-full">
                        <div className="input-group-admin">
                            ...
                            <input type="text" id="name" name="name" value={formData.name} onChange={handleChange} placeholder="e. g. Running" />
                        </div>
                        <div className="input-group-admin">
                            <label htmlFor="description">Description</label>
                            <textarea name="description" id="description" rows={4} placeholder="Category Details" value={formData.description} onChange={handleChange}></textarea>
                        </div>
                    </div>
                </div>
                <Button className="ml-auto mt-3 rounded-lg" type="submit" onClick={handleSubmit} disabled={isSubmitting}>{isEditMode ? "Update Category" : "Create Category"}</Button>
            </form>
        </Modal>
    );
};
